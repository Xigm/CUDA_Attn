...
__global__ void matmul_kernel_batched(float* A, float* B, float* C, int m, int n, int p, int batch_size) {
    ...
    if (x < m && y < p && z < batch_size) {
        ...
        for (int k = 0; k < n; k++) {
            ...
            // while B is z * n * p	
            sum += A[z * m * n + y * n + k] * B[z * n * p + k * p + y];
        }
        // here I add z * m * p to the index to access the correct batch for C
        C[z * m * p + y * p + x] = sum;
        if (sum == 0) printf("%f from %d", sum, z * m * p + y * p + x);

    }
}

__global__ void matmul_kernel_batched_MH(float* A, float* B, float* C, int m, int n, int p, int batch_size, int n_heads) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;  

    int third_dim = blockIdx.z;
    int b = third_dim / n_heads;
    int h = third_dim % n_heads;
    ...
    if (x < m && y < p &&  b < batch_size) {
        ...
        for (int k = 0; k < n; k++) {
            // I simply add z * m * n to the index to access the correct batch for A,
            // while B is z * n * p	
            sum += A[b * m * n * n_heads + h * m * n 
                            + y * n + k] 
                            
                 * B[b * p * n * n_heads + h * p * n 
                            + k * p + x];

        }
        // here I add z * m * p to the index to access the correct batch for C
        C[b * m * p * n_heads + h * m * p + y * p + x] = sum;
        // if (h == 1) printf("%f from %d m %d n %d \n", C[b * m * p * n_heads + h * m * p + y * p + x],b * m * p * n_heads + h * m * p + y * p + x);
...
// Jose'd from https://developer.nvidia.com/blog/efficient-matrix-transpose-cuda-cc/
...
// __global__ void transpose(float *odata, const float *idata, int m, int n, int batch_size)
// {
//     int x = blockIdx.x * blockDim.x + threadIdx.x;
//     int y = blockIdx.y * blockDim.y + threadIdx.y;
//     int b = blockIdx.z * blockDim.z + threadIdx.z;

//     for (int j = 0; j < n; j += BLOCK_ROWS)
//         odata[b * m * n +  x*width + (y+j)] = idata[b * m * n + m * (y + j) + x];
// }

__global__ void atomicTransposeMH(float * A, float * B, int num_inputs, int dk, int n_heads, int batch_size, int head_size) {
    ...
    if (x < head_size && y < num_inputs && b < batch_size) {
        // atomicSet(&B[b * m * n + y * m + x], A[b * m * n + x * n + y]);
        // atomicSet(&B[b * m * n + x * n + y], A[b * m * n + y * m + x]);
        atomicSet(&B[b * num_inputs * dk + h * head_size * num_inputs + y + x * num_inputs], A[b * num_inputs * dk + h * head_size * num_inputs + y * head_size + x]);
        
    }
}

__global__ void organiseMHatomic(float * A, float * B, int num_inputs, int dk, int n_heads, int batch_size, int head_size) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    int b = blockIdx.z * blockDim.z + threadIdx.z;
    ...
    if (y < num_inputs && x < dk && b < batch_size) {
        // if (x == 0 && y == 0) printf("%f %f \n", B[b * m * n + head * head_size * m + y * head_size + x], A[b * m * n + y * m + x]);
        atomicSet(&B[b * num_inputs * dk + head * head_size * num_inputs + y * head_size + (x - head * head_size)], A[b * num_inputs * dk + y * dk + x]);
        // atomicSet(&B[b * num_inputs * dk + y * dk + x], A[b * num_inputs * dk + y * dk + x]);
        // if (x == 0 && y == 0) printf("%f %f \n", B[b * m * n + head * head_size * m + y * head_size + x], A[b * m * n + y * m + x]);
...
float attention(float*** input, int num_inputs, int dk, int batch_size, int n_heads, float*** output, float** Wq, float** Wk, float** Wv, float **W_cproj) {    
    ...
    float* d_input, *d_output, *d_output_2, *d_Wq, *d_Wk, *d_Wv, *d_W_cproj, *attn, *Q, *Q_mh, *K_mh, *V_mh, *K, *K_T, *V;
    ...
    // loadMatrixToGPU_b_repeated(Wq, p_d_Wq, dk, dk, batch_size);
    // loadMatrixToGPU_b_repeated(Wk, p_d_Wk, dk, dk, batch_size);
    // loadMatrixToGPU_b_repeated(Wv, p_d_Wv, dk, dk, batch_size);
    ...
    cudaMalloc(&K_T, batch_size * num_inputs * dk * sizeof(float));
    ...
    cudaMalloc(&Q_mh, batch_size * num_inputs * dk * sizeof(float));
    cudaMalloc(&K_mh, batch_size * num_inputs * dk * sizeof(float));
    cudaMalloc(&V_mh, batch_size * num_inputs * dk * sizeof(float));
    ...
    // cudaMalloc(&attn_MH, batch_size * n_heads * num_inputs * num_inputs * sizeof(float));
    ...
    // print_from_GPU(K_T, num_inputs, dk, batch_size);
    dim3 kernel_attn_blockDim(dk,1,1);
    dim3 kernel_attn_gridDim(1,num_inputs,batch_size);
    ...
    milliseconds = 0;
    CHECK(cudaEventElapsedTime(&milliseconds, start_cuda, stop_cuda));
    printf("Kernel organise Q, V, K MH time: %f ms\n", milliseconds);


    // print_from_GPU(K, num_inputs, dk, batch_size);
    ...
    CHECK(cudaEventRecord(start_cuda, 0));
    atomicTransposeMH<<<transpose_gridDim, transpose_bloqDim>>>(K_mh, K_T, num_inputs, dk, n_heads, batch_size, head_size);
    CHECK(cudaEventRecord(stop_cuda, 0));
    CHECK(cudaEventSynchronize(stop_cuda));
    milliseconds = 0;
    CHECK(cudaEventElapsedTime(&milliseconds, start_cuda, stop_cuda));
    printf("Kernel Transposing time: %f ms\n", milliseconds);

    // print_from_GPU_sm(K_mh, num_inputs, head_size, batch_size, n_heads);
    // print_from_GPU_sm(K_T, head_size, num_inputs, batch_size, n_heads);

    dim3 normal_kernel_for_attn_blockDim(32, 32, 1);
    dim3 normal_kernel_for_attn_gridDim((num_inputs - 1)/32 + 1, (num_inputs - 1)/32 + 1, batch_size * n_heads);

    CHECK(cudaEventRecord(start_cuda, 0));
    matmul_kernel_batched_MH<<<normal_kernel_for_attn_gridDim, normal_kernel_for_attn_blockDim>>>(Q_mh, K_T, attn, num_inputs, head_size, num_inputs, batch_size, n_heads);
    CHECK_KERNELCALL();
    CHECK(cudaEventRecord(stop_cuda, 0));
    CHECK(cudaEventSynchronize(stop_cuda));
    milliseconds = 0;
