...
__global__ void matmul_kernel_semi_tiled(const float*__restrict__ A, const float*__restrict__ B, float*__restrict__ C, int m, int n, int p, int batch_size) {
    ...
    if (x < m && y < p && b < batch_size) {
        ...
        for (int k = 0; k < n; k++) {
            // I simply add z * m * n to the index to access the correct batch for A,
            // while B does not have batch dim
            sum += A[b * m * n + x * n + k] * shared_data[k];
        }
        // printf("%d sum %f \n", x * p + y, sum);
        C[b * m * p + x * p + y] = sum;
...
float attention(float*** input, int num_inputs, int dk, int batch_size, int n_heads, float*** output, float** Wq, float** Wk, float** Wv, float **W_cproj) {    
    ...
    dim3 tiled_kernel_bloqDim(1, num_inputs, 1);
    dim3 tiled_kernel_gridDim(dk, 1, batch_size);
    ...
    matmul_kernel_semi_tiled<<<kernel_MH_gridDim, kernel_MH_bloqDim, dk * sizeof(float)>>>(d_input, d_Wq, Q, num_inputs, dk, dk, batch_size);
